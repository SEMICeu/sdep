...
class AreaFactory(AsyncSQLAlchemyFactory):
    ...
    regulation = "all"  # Default: covers both listing and activity
...
class AddressFactory(factory.Factory):
    ...
    full_address = factory.LazyAttribute(
        lambda o: f"{o.thoroughfare} {o.locator_designator_number}, {o.post_code} {o.post_name}"
    )
...
class ActivityFactory(AsyncSQLAlchemyFactory):
    ...
    address_full_address = factory.LazyAttribute(
        lambda o: f"{o.address_thoroughfare} {o.address_locator_designator_number or ''}, {o.address_post_code} {o.address_post_name}".replace(
            " ,", ","
        )
    )
    registration_number = factory.Sequence(lambda n: f"REG{n:06d}")
    # Foreign key to Area - defaults to creating one
    # Guest information — cardinality constraint: len(country_of_guests) == number_of_guests
    number_of_guests = Faker("random_int", min=1, max=8)
    country_of_guests = factory.LazyAttribute(
        lambda o: ["NLD"] * o.number_of_guests
    )  # ISO 3166-1 alpha-3 or "N/A"; length matches number_of_guests
